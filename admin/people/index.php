...
function store_people_upload($file) {
  ...
  if ($origName === '' || !is_uploaded_file($tmpName)) return null;
  ...
  if (!move_uploaded_file($tmpName, $destAbs)) return null;
...
<body class="admin-body">
  <div class="admin-wrap">
    ...
    <div class="admin-card">
      <h3 style="margin:0 0 6px">Members (Edit / Delete)</h3>
      <table class="admin-table">
        <thead><tr><th>Photo</th><th>Details</th><th>Actions</th></tr></thead>
        <tbody>
          <?php foreach ($rows as $r): ?>
            <tr>
              <td>
                <?php if ((string)$r['image_path'] !== ''): ?>
                  <img src="<?=h(normalize_image_path((string)$r['image_path']))?>" alt="" style="width:64px;height:64px;border-radius:10px;object-fit:cover">
                <?php endif; ?>
              </td>
              <td>
                <form method="post" enctype="multipart/form-data">
                  ...
                  <div class="grid-3">
                    <div><label>Page</label><select name="page_key"><?php foreach ($pageLabels as $k => $label): ?><option value="<?=h($k)?>" <?= ((string)$r['page_key'] === $k ? 'selected' : '') ?>><?=h($label)?></option><?php endforeach; ?></select></div>
                    <div><label>First name</label><input name="first_name" value="<?=h((string)$r['first_name'])?>" required></div>
                    <div><label>Last name</label><input name="last_name" value="<?=h((string)$r['last_name'])?>" required></div>
                    <div><label>Role/Position</label><input name="role_title" value="<?=h((string)$r['role_title'])?>"></div>
                    <div><label>Sort order</label><input type="number" name="sort_order" value="<?= (int)$r['sort_order'] ?>"></div>
                    <div><label>Replace image</label><input type="file" name="image_file" accept=".jpg,.jpeg,.png,.webp"></div>
                  </div>
                  <div style="margin-top:8px"><label>Image path / URL</label><input name="image_path" value="<?=h((string)$r['image_path'])?>"></div>
